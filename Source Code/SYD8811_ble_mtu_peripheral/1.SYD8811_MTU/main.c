...
#define USER_MTU_SIZE 125 //MTU req + 3 byte header
uint8_t mtu_buff[USER_MTU_SIZE] = {0};
uint8_t send_mtu_num = 0;
...
uint8_t BLE_SendData(uint8_t *buf, uint16_t len);
...
void ble_evt_callback(struct gap_ble_evt *p_evt)
{
	...
	{
		#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
			dbg_printf("interval:%x, latency:%x, svto:%x\r\n", p_evt->evt.conn_update_complete_evt.interval,
																												 p_evt->evt.conn_update_complete_evt.latency,
																												 p_evt->evt.conn_update_complete_evt.svto);
		#endif
	}
}
...
{
	if(p_attc->attc_code == ATT_MTU_RSP)
	{
		#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
			dbg_printf("ATT_MTU_RSP MTU Size:%d\r\n", p_attc->attc.AttMtuRsp.mtu);
		#endif
	}
}
...
static void gpio_callback(void)
{
	...
	dbg_printf("IO WAKEUP INT\r\n");
	
	if(SW&U32BIT(KEY1))//请求MTU 看手机RSP
	{
		if(GPIO_Pin_Read(U32BIT(KEY1))&U32BIT(KEY1))
		{
//			dbg_printf("KEY1 IO INT VALID\r\n");
			
			if(connect_flag)
			{
				ATTCMTUReq(USER_MTU_SIZE+3);
				dbg_printf("ATTCMTUReq MTU Size 128 byte\r\n");
			}
			GPIO_Pin_Turn(U32BIT(LED1));
		}
	}
	else if(SW&U32BIT(KEY2))//发送数据测试
	{
		if(GPIO_Pin_Read(U32BIT(KEY2))&U32BIT(KEY2))
		{
//			dbg_printf("KEY2 IO INT VALID\r\n");
			GPIO_Pin_Turn(U32BIT(LED2));
			Timer_Evt_Start(EVT_10MS);
		}
	}
}

void key2_timer_handle(void)
{	
	if(GPIO_Pin_Read(U32BIT(KEY2)) == U32BIT(KEY2))//配置成输入上拉 作为唤醒源需要取反 这里按下就读取到1了
	{
		
		key_loop ++;
//		if(key_loop == 200) //长按2s
//		{
//			DBGPRINTF("key 2 Long Press\r\n");
//		}
	}
	else
	{
		...
		{
			DBGPRINTF("key 2 Short Press\r\n");
			
			if(connect_flag)
			{
				memset(mtu_buff, 0x55, USER_MTU_SIZE);
				mtu_buff[0] = send_mtu_num;
				mtu_buff[USER_MTU_SIZE-1] = 0xAA;
				send_mtu_num ++;
				DBGPRINTF("SendData 125 byte\r\n");
				BLE_SendData(mtu_buff, USER_MTU_SIZE);
			}
		}
		key_loop = 0;
		Timer_Evt_Stop(EVT_10MS);
  } 
}
...
//				Connection_latency();   //连接参数相关设置
			...
			#ifdef	EVT_10MS
			if(TIMER_EVT&EVT_10MS)   
			{
				Timer_Evt_Clr(EVT_10MS);
				#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
//				DBGPRINTF(("EVT_10MS\r\n"));
				#endif
