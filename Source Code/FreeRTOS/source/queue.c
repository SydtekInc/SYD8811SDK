...
{
	__align(4) Queue_t * const pxQueue = ( Queue_t * ) xQueue;//1
...
{
__align(4) Queue_t *pxNewQueue;//1
__align(4) size_t xQueueSizeInBytes;//1
QueueHandle_t xReturn = NULL;
__align(4) int8_t *pcAllocatedBuffer;//1
...
/*-----------------------------------------------------------*/
	...
	{
	BaseType_t xReturn;
	__align(4) Queue_t * const pxMutex = ( Queue_t * ) xMutex;//1
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xEntryTimeSet = pdFALSE;
	TimeOut_t xTimeOut;
	__align(4) int8_t *pcOriginalReadPosition;//1
	__align(4) Queue_t * const pxQueue = ( Queue_t * ) xQueue;//1
		...
		for ( ;; )
		{
			taskENTER_CRITICAL();
			{
				if ( pxQueue->uxMessagesWaiting > ( UBaseType_t ) 0 )
				{
					...
					dbg_printf("pxQueue-->%d\r\n", ( void * )pxQueue);
					dbg_printf("pvBuffer-->%d\r\n", ( void * )pvBuffer);
					dbg_printf("( size_t ) pxQueue->uxItemSize-->%d\r\n", ( size_t ) pxQueue->uxItemSize);
...
//				dbg_printf("( size_t ) pxQueue->uxItemSize-->%d\r\n", ( size_t ) pxQueue->uxItemSize);
				...
				prvCopyDataFromQueue( pxQueue, pvBuffer );//--1,pxQueue algin;pvBuffer 
...
{
BaseType_t xReturn;
__align(4) UBaseType_t uxSavedInterruptStatus;//1
__align(4) Queue_t * const pxQueue = ( Queue_t * ) xQueue;//1
...
//		dbg_printf("\r\n%s-->%s-->%d\r\n", __FILE__, __FUNCTION__, __LINE__);
//		
//		dbg_printf("( void * ) pxQueue->pcWriteTo-->%d\r\n", ( void * ) pxQueue->pcWriteTo);
//		dbg_printf("pvItemToQueue-->%d\r\n", pvItemToQueue);
//		dbg_printf("( size_t ) pxQueue->uxItemSize-->%d\r\n", ( size_t ) pxQueue->uxItemSize);
...
//		dbg_printf("pvBuffer-->%d\r\n", ( void * )pvBuffer);
//		dbg_printf("pvBuffer-->%d\r\n", ( void * )pvBuffer);
//		dbg_printf("( void * ) pxQueue->u.pcReadFrom-->%d\r\n", ( void * ) pxQueue->u.pcReadFrom);
//		dbg_printf("( size_t ) pxQueue->uxItemSize-->%d\r\n", ( size_t ) pxQueue->uxItemSize);
