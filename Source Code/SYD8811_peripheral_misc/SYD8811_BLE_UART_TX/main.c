...
static void ble_gatt_write(struct gap_att_write_evt evt)
{
	if(evt.uuid == BLE_UART_Write_UUID)
	{
		...
		#ifdef UART_BLE_DUPLX
		BLE_SendUARTData(evt.data, evt.sz);
		#endif
	}
//	else if(evt.uuid== BLE_OTA_Read_Write_UUID)
//	{
...
void Start_Send_to_Ble(void){
	if(uart_rx_buf.header==0)
	{
		#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
		DBGPRINTF("No receive anything from BLE ");
		#endif
	}
	else
	{
		#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
		DBGPRINTF("Receive %dByte from BLE ",uart_rx_buf.header);
		#endif
		if(memcmp(uart_rx_buf.data,data,20)==0)
		{
			 ok_cnt++;
			#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
			DBGPRINTF("Compare S/R OK!  ",uart_rx_buf.header);
			#endif
		}
		else
		{
			faile_cnt++;
			#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
			DBGPRINTF("Compare S/R Faile!\r\n",uart_rx_buf.header);
			DBGHEXDUMP("RxData:\r\n",uart_rx_buf.data,20);
			#endif
		}
	}
	#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
	DBGPRINTF("Send 0X00-0X13(20Byte) num ok:%d faile:%d\r\n",ok_cnt,faile_cnt);
	#endif
	uart_rx_buf.header = 0;
	uart_rx_buf.tail = 0;
	memset(uart_rx_buf.data,0xff,20);
	uart_cmd(data, 20);
}
...
int main(void)
{
	...
	while(1)
	{
		...
		{
			#ifdef	EVT_1S_UART
			if(TIMER_EVT&EVT_1S_UART)   
			{
				timer500ms_cnt++;
				if(timer500ms_cnt>20)
				{
					Start_Send_to_Ble();
					timer500ms_cnt=0;
				}
				Timer_Evt_Clr(EVT_1S_UART);
			}
			#endif
			#ifdef	EVT_2S
			if(TIMER_EVT&EVT_2S)
