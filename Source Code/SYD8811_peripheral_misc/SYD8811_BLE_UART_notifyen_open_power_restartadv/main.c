...
static void setup_adv_data_fast()
{
	struct gap_adv_params para;
		
	memset(&para, 0x0, sizeof(struct gap_adv_params));
	para.channel = 0x07;	//0x07: 37&38&39	
	para.type = ADV_IND;
	para.policy = 0x0;
	para.interval = 0x0320;//500ms
	para.timeout = 0x0001;	//to=1s, calc the rnd > 1
...
{	
	...
	else if(p_evt->evt_code == GAP_EVT_ADV_END)
	{
		...
		if(connect_flag & BIT2)
		{
			DBGPRINTF(("Fast adv end StartAdv normal\r\n"));
			setup_adv_data();
			StartAdv();
			connect_flag &=~BIT2;
		}
		else if(!(connect_flag & BIT1))
		{
			DBGPRINTF(("re StartAdv\r\n"));
			StartAdv();
		}
		else
		{
			DBGPRINTF(("adv exception state\r\n"));
		}
	}
}
...
{
    #if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
		dbg_printf("gpio callback %x\r\n",connect_flag);
	#endif
	if(!(connect_flag & BIT0))  //广播状态
	{
		if(connect_flag & BIT1)  //关闭广播状态
		{
			RFWakeup();
			DelayMS(100);
			UartEn(true);
			ble_init();   //要保证4k_setting里面的时钟的设置和本工程的设置一样，否则ble_init函数后定时器可能不能够工作
			UartEn(false);
			DelayMS(100);
				...
				dbg_printf("StartAdv\r\n");
			#endif
			setup_adv_data_fast();  //设置一个超时时间比较短的广播参数，更加快的让ADV_EN事件到来，ADV_EN事件到来，到来后开启正常的广播
			StartAdv();  //再次广播
			connect_flag &=~BIT1;
			connect_flag |=BIT2;   //再次广播状态
		}
		else //正在广播状态
		{			
			#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
			...
			#endif
			StopAdv();
			connect_flag |=BIT1;   //关闭广播状态
			connect_flag &=~BIT2;
		}
		delay_ms(1000);   //固定延时，消斗
	}
}
