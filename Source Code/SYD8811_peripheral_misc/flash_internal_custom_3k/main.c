...
{	
	...
	while(1)
	{
		if(GPIO_Pin_Read( U32BIT(KEY1)))
		{
			...
			WriteProfileData(FLASH_INTERNAL_DATA_ADDR+addr,WRITE_READ_LEN, tx_buff); //注意：这是3Kflash的操作，使用WriteProfileData函数会自动擦除，
		...
		if(GPIO_Pin_Read( U32BIT(KEY2)))
		{
			...
			ReadProfileData(FLASH_INTERNAL_DATA_ADDR+addr,WRITE_READ_LEN, rx_buff);
