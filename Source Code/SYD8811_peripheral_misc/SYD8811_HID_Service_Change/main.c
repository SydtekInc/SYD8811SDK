...
static uint8_t service_change_tx=0;
uint8_t service_change_state=0;
...
//发送函数
uint8_t BLE_Indicate_Service_Changed(uint16_t start, uint16_t end)
{
	if(service_change_tx == 1)
	{
		uint8_t data[4]={0};
		struct gap_att_report report;
		data[0]=start%256;
		...
		data[2]=end%256;
		data[3]=end/256;
		report.primary = BLE_GENERIC_ATTRIBUTE;
		report.uuid = BLE_SERVICE_CHANGED_UUID;
		report.hdl = BLE_SERVICE_CHANGED_VALUE_HANDLE;					
		report.value = BLE_GATT_INDICATION;
		return GATTDataSend(BLE_GATT_INDICATION, &report, 4, data);
	}
	return 0;
}
...
void ble_evt_callback(struct gap_ble_evt *p_evt)
{
	...
	{					
		...
		else if(p_evt->evt.att_handle_config_evt.uuid == BLE_GENERIC_ATTRIBUTE)
		{
			if(p_evt->evt.att_handle_config_evt.value & BLE_GATT_INDICATION)
			{
				#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_) 
				DBGPRINTF(("GENERIC_ATTRIBUTE enable\r\n"));
				#endif
				service_change_tx=1;
			}
			else
			{
				#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_) 
				DBGPRINTF(("GENERIC_ATTRIBUTE disable\r\n"));
				#endif
				service_change_tx=0;
			}
		}
    ...
    if(GPIO_Pin_Read(U32BIT(KEY3)))
		{
				...
				if(GPIO_Pin_Read(U32BIT(KEY3)))
				{
						...
						if(service_change_tx==1)
						{
							if((service_change_state & BIT0)==0)
							{
								{
									uint8_t enc=0,hld=0x2c;
									if(APP_Read_Attribute_Encrypt(hld,&enc))
									{
										#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
										DBGPRINTF("Find hld:%x enc:%x\r\n",hld,enc);
										#endif
										if(enc==0x10)
										{
											enc=0X70;
											APP_Write_Attribute_Encrypt(hld,&enc);
											#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
											DBGPRINTF("Write hld:%x enc:%x\r\n",hld,enc);
											#endif
											if(BLE_Indicate_Service_Changed(0x19, 0xFFFF))
											{
												service_change_state |=BIT0;
											}
											#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_) 
													DBGPRINTF("Indicate_Service_Changed\r\n");
											#endif
										}
									}
									else
									{
										#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
										DBGPRINTF("Can not Find enc hld:%x\r\n",hld);
										#endif
									}
//									hld=0x2b;
//									if(APP_Read_Charact_Permission(hld,&enc))
//									{
//										#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
//										DBGPRINTF("Find hld:%x Permission:%x\r\n",hld,enc);
//										#endif
//									}
//									else
//									{
//										#if defined(_DEBUG_) || defined(_SYD_RTT_DEBUG_)
//										DBGPRINTF("Can not Find Permission hld:%x\r\n",hld);
//										#endif
//									}
								}
