...
//HID发送函数
//键值发送函数
uint8_t HID_Key_Byte_Send(uint8_t key_value)
{
	...
	if(hid_key_tx == 1)
	{
		...
		report.hdl = BLE_Consumer_REPORT_VALUE_HANDLE;					
		report.value = BLE_GATT_NOTIFICATION;
    return GATTDataSend(BLE_GATT_NOTIFICATION, &report, 1, &key_value);
	}
	return 0;
}
...
{	
	...
	pw_cfg.gpi_wakeup_cfg =  U32BIT(KEY1) | U32BIT(KEY2) | U32BIT(KEY3); 
...
{	
	...
	{				
		...
		if(!GPIO_Pin_Read(U32BIT(KEY1)))
		{
			...
			#endif
			HID_Key_Byte_Send(PLAY_PAUSE_KEY_BIT);
		...
		if(!GPIO_Pin_Read(U32BIT(KEY2)))
		{
			...
			#endif
			HID_Key_Byte_Send(FORWARD_KEY_BIT);
		...
		if(!GPIO_Pin_Read(U32BIT(KEY3)))
		{
			...
			#endif
			HID_Key_Byte_Send(BACK_KEY_BIT);
			while(!GPIO_Pin_Read(U32BIT(KEY3)));
		}
		else
		{
			//需要再发送一个0，作用类似于释放按键。
