...
{	
	...
	else if(p_evt->evt_code == GAP_EVT_PAIRING_COMP)
	{
		...
		for(i=0;i<MAX_BOND_DEVICE;i++)
		{
			uint8_t ret=0;
			 struct gap_bond_dev device;
			 SetBondManagerIndex(i);
			 ret=GetBondDevice(&device);
			if(ret==0)
			{
				dbg_printf("slot %x not Bond info\r\n",i);
				break;
			}
			else
			{
				dbg_printf("slot %x addr:%x %x %x %x %x %x\r\n",i,device.addr.addr[0],device.addr.addr[1],device.addr.addr[2],device.addr.addr[3],device.addr.addr[4],device.addr.addr[5]);
			}
		}
		if(i>=MAX_BOND_DEVICE)
		{
			ReadProfileData(0 , 1, &i);
			i=(i+1)%MAX_BOND_DEVICE;
			WriteProfileData(0 , 1, &i);
			SetBondManagerIndex(i);
			dbg_printf("slot all have Bond info not use slot %x\r\n",i);
		}
	}
