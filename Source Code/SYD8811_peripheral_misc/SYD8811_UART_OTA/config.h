...
#define	GPIO_UART0_TX	GPIO_16
#define	GPIO_UART0_RX	GPIO_15

#define	GPIO_UART1_TX	GPIO_6
#define	GPIO_UART1_RX	GPIO_7


#define WAKEUP_PIN		BIT10
...
//自定义串口接收通信协议
#define UART_MAX_RX_BUFF_SIZE  256
typedef struct _uart_rx_protocol_{
	uint8_t	all_len; 		    //uart_buf的数据大小
	uint8_t	current_rx_len; //当前接收的数据大小
	uint8_t	flag;           //0:无数据, 1:正在接收数据, 2:接收完数据
	uint8_t receive_buff[UART_MAX_RX_BUFF_SIZE];//第一个字节是长度 = all_len,处理数据时需要注意
}Uart_RX_Protocol ;


//自定义串口通信协议
#define UART_MAX_TX_BUFF_SIZE  256

extern volatile Uart_RX_Protocol uart_rx_buff;
