...
int main(void)
{
	...
	flash_data_sz=flash_data_size_get();
	dbg_printf("flash_data_size %d\r\n",flash_data_sz);
	__enable_irq();	
	...
	while(1)
	{
		...
		sendToUart();	//通过串口发送数据
		if((ota_state==1) || (ota_state==7))ota_manage();
		
		if(GPIO_Pin_Read( U32BIT(KEY1)))
		{
			flash_data_sz+=0x2000;
			flash_data_size_set(flash_data_sz);   //传入的参数必须为0x2000的整倍数
			dbg_printf("flash_data_size setto:%d\r\n",flash_data_sz);
			dbg_printf("flash_data_size now %d\r\n",flash_data_size_get());
			delay_ms(1000*10);
		}
